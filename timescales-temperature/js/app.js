...
var App = (function() {
  ...
  App.prototype.loadLabels = function(labels, domain){
    // var $el = $('#scale-labels');
    //
    // _.each(labels, function(label){
    //   var p = UTIL.norm(label.date, domain[0], domain[1]);
    //   var left = p * 100;
  ...
  App.prototype.onDataLoaded = function(data){
    ...
    // check for labels
    if (_.has(d, "labels")) {
      this.loadLabels(d["labels"], this.maxDomain);
    }
  ...
  App.prototype.simplify = function(data){
    var simplified = data;
    // var tolerance = data.length / 365.0 / 10.0;
